...
import com.zy.p2p.business.domain.*;
import com.zy.p2p.business.mapper.*;
...
import com.zy.p2p.business.service.ISystemAccountService;
import com.zy.p2p.business.util.CalculatetUtil;
import com.zy.p2p.business.util.DecimalFormatUtil;
...
public class BidRequestServiceImpl implements IBidRequestService, ApplicationListener<ContextRefreshedEvent> {
    ...
    public void apply(BidRequest br) {
        ...
        if (this.canApplyBidRequeset(UserContext.getCurrent().getId())
                && br.getBidRequestAmount() != null
                && br.getCurrentRate() != null
                && br.getMinBidAmount() != null
                && br.getBidRequestAmount().compareTo(BidConst.SMALLEST_BIDREQUEST_AMOUNT) >= 0// 系统最小借款金额（500）<=借款金额
                && br.getBidRequestAmount().compareTo(account.getRemainBorrowLimit()) <= 0// 借款金额<=剩余信用额度
                && br.getCurrentRate().compareTo(BidConst.SMALLEST_CURRENT_RATE) >= 0// 5 <= 利息
    ...
    public void bid(Long bidRequestId, BigDecimal amount) {
        ...
        if (br != null// 1,借款存在;
                ...
                && !br.getCreateUser().getId().equals(UserContext.getCurrent().getId())// 3,当前用户不是借款的借款人;
            ...
            if (state == BidRequestAuditHistory.STATE_AUDIT) {
                ...
                for (int i = 1; i <= br.getBids().size(); i++) {
                    ...
                    if (i < br.getBids().size()) {
                        // 待收总利息=投标金额/借款总金额*借款总利息（不是利率）
                        bidInterest = bid.getAvailableAmount().divide(br.getBidRequestAmount(), BidConst.CAL_SCALE, RoundingMode.HALF_UP)
                ...
                // ***2.3.2生成支付借款手续费流水;
                this.accountFlowService.borrowChargeFee(manageChargeFee, br, borrowAccount);
                // ***2.3.3平台收取借款手续费;
    ...
    private void createPaymentSchedules(BidRequest br) {
        ...
        for (int i = 0; i < br.getMonthes2Return(); i++) {
            ...
            if (i < br.getMonthes2Return() - 1) {
                // 计算这一期的总还款金额
                ps.setTotalAmount(CalculatetUtil.calMonthToReturnMoney(br.getReturnType(), br.getBidRequestAmount(), br.getCurrentRate(), i + 1, br.getMonthes2Return()));
                // 计算这一期的利息
                ps.setInterest(CalculatetUtil.calMonthlyInterest(br.getReturnType(), br.getBidRequestAmount(), br.getCurrentRate(), i + 1, br.getMonthes2Return()));
                // 计算这一期的本金
    ...
    private void createPaymentScheduleDetail(PaymentSchedule ps, BidRequest br) {
        ...
        for (int i = 0; i < br.getBids().size(); i++) {
            ...
            if (i < br.getBids().size() - 1) {
                // 计算利息
                BigDecimal interest = DecimalFormatUtil.formatBigDecimal(bid.getAvailableAmount().divide(br.getBidRequestAmount(), BidConst.CAL_SCALE, RoundingMode.HALF_UP).multiply(ps.getInterest()), BidConst.STORE_SCALE);
                // 计算本金
                BigDecimal principal = DecimalFormatUtil.formatBigDecimal(bid.getAvailableAmount().divide(br.getBidRequestAmount(), BidConst.CAL_SCALE, RoundingMode.HALF_UP).multiply(ps.getPrincipal()), BidConst.STORE_SCALE);
    ...
    public void doReturnMoney(Long id) {
        ...
        // ***1,还款对象的状态处于待还状态;
        // ***2,还款的金额<=账户余额;
        if (ps.getState() == BidConst.PAYMENT_STATE_NORMAL && borrowAccount.getUsableAmount().compareTo(ps.getTotalAmount()) >= 0) {
            // 1,针对还款对象;
            // ***1.1,修改还款对象状态;设置属性;
            ...
            for (PaymentScheduleDetail psd : ps.getPaymentScheduleDetails()) {
                ...
                this.accountFlowService.interestChargeFee(psd, interestChargeFee, bidAccount);
                // ***3.5,系统账户收到利息管理费,生成收款流水;
                this.systemAccountService.chargeInterestFee(psd, interestChargeFee);
